...
Merge eligible open pull requests that I created, processing them in ascending PR-number order.
...
- Process my pull requests from the lowest PR number to the highest. Do not skip ahead while an earlier eligible PR is unresolved.
...
2. Fetch and prune `origin`, then list all open pull requests with at least their number, author, head branch, draft status, mergeability, review state, checks, and URL.
3. Classify every open pull request as mine or not mine, and order my pull requests by ascending number.
4. For each eligible pull request of mine, in that order:
   ...
   10. Fetch `origin/main` again before processing the next pull request.
5. Query GitHub again for all open pull requests so the final report reflects the repository's current state.

## Output

Report every pull request considered in a Markdown table with these columns:

| PR | Title | Author | Status | Reason | URL |
| --- | --- | --- | --- | --- | --- |
