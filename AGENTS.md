...
- Weekly Maintenance tactics are rolling intake buckets for work that arrives during the current ISO week. Agents must not mark them `finished`, `accomplished`, `closed`, or `archived` merely because the currently visible task graph is complete; they remain open for additional same-week maintenance tasks. Do not infer weekly-tactic completion from child-task convergence. Close the previous week's Maintenance tactic only after the ISO week has rolled over, using the human-owned lifecycle action, and use the new current-week tactic for subsequent work.
- For non-weekly tactics, agents must mark a completed tactic as `finished` when its task chain is complete, and are explicitly forbidden from accomplishing or archiving tactics. Accomplishing and archiving are human-owned lifecycle actions.
...
- Write every Ecelyo task title, description, acceptance criterion, and related task text in English, regardless of the language used in the request.
