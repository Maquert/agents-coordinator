...
15. If complete and the task's pull request is already merged, first ensure the corresponding Ecelyo task state is updated to `finished`, then add concise completion notes, move it to the finished state requested by the repository workflow, and **delete** `~/.agents/tasks/<task-id>.md`.
