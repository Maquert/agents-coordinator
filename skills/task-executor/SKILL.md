...
20. If a pull request is merged, the corresponding Lylat task state must also be updated to `finished`. A merged PR is not fully reflected until Lylat tracks that change.
...
15. If complete and the task's pull request is already merged, first ensure the corresponding Lylat task state is updated to `finished`, then add concise completion notes, move it to the finished state requested by the repository workflow, and **delete** `~/.agents/tasks/<task-id>.md`.
