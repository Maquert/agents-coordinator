...
description: Prompt, ChatGPT context, and skill cost-efficiency coaching for Codex. Use when the user explicitly requests AI efficiency or cost optimization, or asks Codex to audit or improve prompts, ChatGPT conversations, custom GPT instructions, AGENTS.md instructions, Codex skills, AI workflows, LLM API usage, context-window packing, token budgets, prompt caching, structured prompt data, TOON, RTK Query-like caching/deduplication, retrieval, memory hygiene, or model/cost tradeoffs. Do not use for ordinary coding, file edits, git operations, or unrelated sessions. Provides one cheap, practical, varied suggestion without taking over the main task.
---
