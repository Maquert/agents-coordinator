...
9. Format that deep link as a normal clickable Markdown link whenever possible, for example `[Open in Codex](codex://threads/<thread-id>)`.
...
12. Do not add any extra agent-branding prose or custom footer text to the PR body, such as `Generated by Claude Code`. The `## Talk to the agent` section, labels, and branch name are sufficient agent-identifying context.
13. Return the PR number and URL.
14. Never ask the user to review a draft pull request. If user review is needed, convert the PR to ready-for-review first or create it as a non-draft PR.
...
When finalizing any PR body, ensure `## Talk to the agent` is the last section in the document and that its body includes the deep link as a normal clickable Markdown link whenever possible.
